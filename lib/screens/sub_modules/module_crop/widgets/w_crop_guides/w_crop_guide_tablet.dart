...
import 'package:passport_photo_2/screens/sub_modules/module_crop/widgets/w_bottom_buttons.dart';
import 'package:passport_photo_2/screens/sub_modules/module_crop/widgets/w_dialog_tooltip.dart';
import 'package:passport_photo_2/screens/sub_modules/module_crop/widgets/w_dialog_unit_body.dart';
import 'package:passport_photo_2/screens/sub_modules/module_crop/widgets/w_dot.dart';
import 'package:passport_photo_2/screens/sub_modules/module_crop/widgets/w_infor_box_crop_guide.dart';
...
class WCustomCropGuideTablet extends StatefulWidget {
  ...
  @override
  State<WCustomCropGuideTablet> createState() => _WCustomCropGuideTabletState();
}

class _WCustomCropGuideTabletState extends State<WCustomCropGuideTablet>
    with WidgetsBindingObserver {
  final double _sizeBlueCursor = 24.0;
  final double _heightEyes = 40.0;
  final double _heightHeadChin = 12.0;
  final double _heightTooltip = 40.0;
  final double _ratioHeadToChinOnMan = 0.345;
  final double _sizeDot = 6;
  Offset offsetZero = const Offset(0, 0);
  final double _mainRatio = 0.23;

  late Size _size, _mainSize;
  ...
  late TextEditingController _controllerWidth,
      _controllerHeight,
      _controllerWidthPrevious,
      _controllerHeightPrevious;
  ...
  void initState() {
    ...
    _controllerWidth =
        TextEditingController(text: selectedPassport.width.toString());
    _controllerWidthPrevious =
        TextEditingController(text: selectedPassport.width.toString());
    _controllerHeight =
        TextEditingController(text: selectedPassport.height.toString());
    _controllerHeightPrevious =
        TextEditingController(text: selectedPassport.height.toString());
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      _updateOffsets(selectedPassport.ratioHead, selectedPassport.ratioEyes,
  ...
  void _updateImageSize(double width, double height) {
    double abc = min(_size.width, _size.height);
    _imageSizeOriginal ??= Size(abc * _mainRatio, abc * _mainRatio);
    double newWidth, newHeight;
    newHeight = newWidth = abc * _mainRatio;
    double ratioWH = 1;
    if (width == 0 || height == 0) {
      ratioWH = 0;
    } else {
    ...
    consolelog("_imageSize ${_imageSize}");
  }

  void _onSave() {
    final width = double.parse(_controllerWidth.text.trim());
    final height = double.parse(_controllerHeight.text.trim());
  ...
  void _onPanUpdate(DragUpdateDetails details) {
    ...
    switch (_indexFacePositionSelected) {
      ...
      case 1: // eyes : khong vuot qua head, chin
        ...
        double limitMax = _offsetChin.dy -
            _heightHeadChin -
        ...
        _offsetBlueCursor = _offsetEyes.translate(
            ...
            (_heightEyes / 2) -
                _sizeBlueCursor /
                    2); // tinh tien ve vi tri ban dau cua cursor va vao giua cua eyes
        setState(() {});
        break;
      case 2: // chin
  ...
  void _onCheckOffset(Offset globalPosition) {
    ...
    final endOffsetCursor = startOffsetCursor.translate(
        _renderBoxHead.size.width, _renderBoxCursor.size.height);
    if (containOffset(
      globalPosition,
    ...
    final endOffsetHead = startOffsetHead.translate(
        _renderBoxHead.size.width, _renderBoxHead.size.height);
    if (containOffset(
      globalPosition,
    ...
    final endOffsetChin = startOffsetChin.translate(
        _renderBoxChin.size.width, _renderBoxChin.size.height);
    if (containOffset(
      globalPosition,
    ...
    final endOffsetEyes = startOffsetEyes.translate(
        _renderBoxEyes.size.width, _renderBoxEyes.size.height);
    if (containOffset(
      globalPosition,
  ...
  void _onChangeOffsetBlueCursor(int index) {
    switch (index) {
      ...
      case 1:
        _offsetBlueCursor =
            _offsetEyes.translate(0, -_sizeBlueCursor / 2 + _heightEyes / 2);
      case 2:
        _offsetBlueCursor = _offsetChin.translate(0, -3);
      default:
    }
    ...
    if (_renderImage == null) {
      ...
      switch (index) {
        ...
        case 1: // chin
          ...
          offset1 =
              Offset(0, _offsetEyes.dy + (imageHeight - _offsetEyes.dy) / 2);
          break;
        case 2: // eyes
          percent0 = (_offsetChin.dy + _heightHeadChin - _offsetHead.dy).abs() /
              imageHeight;
  ...
  void _showDialogTooltip({
    ...
    Offset viTriGiuaBenTraiTooltip =
        startOffset.translate(0, renderBoxTooltip.size.height / 2);
            ...
            onDone: (newPercent, newUnitValue) {
              ...
              switch (_indexFacePositionSelected) {
                case 0:
                      ...
                      mainPercent = max(
                          ...
                          min(
                              ...
                              (listValueEyes[0][0] -
                                  _heightHeadChin /
                                      imageHeight *
                                      100))); // tru di % height cua head

                      break;
                    case 1: //ok
                      double pMax = listValueHead[0][0] + listValueHead[1][0];
                      double limitEyes = listValueEyes[0][0] -
                  ...
                  _offsetBlueCursor =
                      _offsetHead.translate(0, -_heightHeadChin);
                  break;
                case 1:
                  double mainPercent = listValueEyes[0][0];
                  switch (indexTooltip) {
                    case 0: //ok
                      double pHead = listValueHead[0][0] +
                          _heightHeadChin / imageHeight * 100;
                      double pChin = 100.0 -
                          (listValueChin[1][0] +
                              _heightHeadChin / imageHeight * 100);
                  ...
                  _offsetEyes = Offset(
                      0, (imageHeight * mainPercent / 100) - _heightEyes / 2);
                  _offsetBlueCursor = _offsetEyes.translate(
                      0, _heightEyes / 2 - _sizeBlueCursor / 2);
                  break;
                case 2:
                  ...
                  switch (indexTooltip) {
                    case 0: //ok
                      ...
                      double pEyes = listValueEyes[0][0] +
                          (_heightHeadChin - _heightEyes / 2) /
                              imageHeight *
                              100;
                      mainPercent = min(pMax, max(newPercent, pEyes)) +
                          listValueHead[0][0];
                      break;
                    case 1: //ok
                      double pMin = listValueEyes[0][0] +
                  ...
                  _offsetChin = Offset(
                      0, (imageHeight * mainPercent / 100) - _heightHeadChin);
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    switch (state) {
      case AppLifecycleState.resumed:
        consolelog('AppLifecycleState.resumed');
        if (mounted) {
          final selectedPassport = widget.countrySelected
              .listPassportModel[widget.countrySelected.indexSelectedPassport];
          _updateImageSize(selectedPassport.width, selectedPassport.height);
      ...
      case AppLifecycleState.inactive:
        consolelog('AppLifecycleState.inactive');
        break;
      case AppLifecycleState.paused:
        consolelog('AppLifecycleState.paused');
        break;
      case AppLifecycleState.detached:
        consolelog('AppLifecycleState.detached');
        break;
      case AppLifecycleState.hidden:
        consolelog('AppLifecycleState.hidden');
        break;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    _mainSize = Size(min(SIZE_EXAMPLE.width, _size.width), _size.height);
    return WillPopScope(
      onWillPop: () async {
        return false;
      },
      child: Scaffold(
        ...
        body: Center(
          child: Container(
            height: min(_size.height, _size.width) * 0.95,
  ...
  Widget _buildFaceOptions() {
    return AnimatedOpacity(
      ...
      child: Container(
        ...
        width: min(_size.width, _size.height) * (_mainRatio + 0.1),
        child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: LIST_POSITION_FACE.indexed.map((e) { 
              return _buildFaceOptionItem(
                  e.$1 == _indexFacePositionSelected, e.$2, () {
                _indexFacePositionSelected = e.$1;
                _onChangeOffsetBlueCursor(_indexFacePositionSelected);
                setState(() {});
              });
            }).toList()),
  ...
  Widget _buildTooltipWidget() {
    List listValue =
        _handleCaculateValuesAndOffsets(_indexFacePositionSelected);
    double percent0, percent1, value0, value1;
  ...
  Widget _buildEditTextFieldWidget(
    ...
    Unit currentUnit,
    Function(RenderBox renderBox) onTapUnitWidget,
  ) {
    return SizedBox(
                ...
                child: TextField(
                  ...
                  onSubmitted: (value) {
                    ...
                    if (value.trim() == "") {
                      ...
                      _value = double.parse(value.trim());
                    }
                    if (_value >= 0) {
                      controllerPrevious.text = _value.toString();
                      _onSubmitted();
                    } else {
                      controller.text = controllerPrevious.text.trim();
                    }
                    setState(() {
                      _avoidResizeKeyboard = false;
                    });
                ...
                child: Row(
                  children: [
                    ...
                    Icon(FontAwesomeIcons.caretDown,
                        size: 12, color: !_isDarkMode ? white : black)
                  ],
                )),
          )
        ],
  ...
  Widget _buildEyesWidget() {
    return Positioned(
      ...
      child: Container(
        width:
            min(_size.width, _size.height) * _mainRatio, //  _imageSize.width,
        height: _heightEyes,
        decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(10),
            border: Border.all(
  ...
  Widget _buildDashLine() {
    return SizedBox(
      width: min(_size.width, _size.height) * _mainRatio, //  _imageSize.width,
      child: WLineDash(
        color: _isDarkMode ? primaryDark1 : primaryLight1,
      ),
    );
